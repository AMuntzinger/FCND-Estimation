...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  // state: x, y, z, x_dot, y_dot, z_dot, yaw
  ...
  // rotate vector from body frame to inertial frame
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  // "Estimation for Quadrotors" eq. 52 
  float theta = pitch;
  float phi = roll;
  float psi = yaw;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  // "Estimation for Quadrotors" eq. 51 
  gPrime(0, 3) = dt;
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  for (int i = 0; i < 6; i++)
  {
      // "Estimation for Quadrotors" eq. 53, 54 
      zFromX(i) = ekfState(i);
      // "Estimation for Quadrotors" eq. 55 
      hPrime(i, i) = 1;
  }
